let FgGallery = function (selector) {
  ...
  if (!document.querySelector(".body-cover")) {
    ...
    bodyCoverDeps = document
      .createRange()
      .createContextualFragment(bodyCoverDeps).firstElementChild;
  ...
  // Adding cols class
  this.gallerySelector.forEach((i) =>
    i.classList.add(`cols-${this.argumens[1].cols}`)
  );
...
FgGallery.prototype.generateItems = function () {
  ...
  this.gallerySelector.forEach(function (galleryContainer, i) {
    ...
    galleryContainer
      ...
      .forEach(function (imageItems, key) {
        ...
        newImages = galleryContainer.appendChild(
          document.createRange().createContextualFragment(newImages)
            .firstElementChild
        );
        ...
        if (window.screen.width > 710) {
          newImages.onclick = function () {
            ...
            This.activeImage =
              This.originalImages[`gallery${i}`][key].cloneNode(true);
        ...
        function imagePopResize(elem) {
          if (
            elem.naturalWidth < window.innerWidth &&
            elem.naturalHeight < window.innerHeight
          ) {
...
FgGallery.prototype.next = function () {
  this.nextBtn.onclick = () => {
    ...
    if (
      this.galleryItemSequence <
      this.originalImages[`gallery${this.gallerySequence}`].length
    ) {
      this.activeImage.src =
        this.originalImages[`gallery${this.gallerySequence}`][
          this.galleryItemSequence
        ].src;
    } else {
      this.galleryItemSequence = 0;
      this.activeImage.src =
        this.originalImages[`gallery${this.gallerySequence}`][0].src;
    }
  };
};
...
FgGallery.prototype.back = function () {
  this.prevBtn.onclick = () => {
    ...
    if (this.galleryItemSequence > -1) {
      ...
      this.galleryItemSequence =
        this.originalImages[`gallery${this.gallerySequence}`].length - 1;
      this.activeImage.src =
        this.originalImages[`gallery${this.gallerySequence}`][
          this.galleryItemSequence
        ].src;
...
FgGallery.prototype.styles = function () {
  if (typeof this.argumens[1].style) {
    for (const key in this.argumens[1].style) {
      document
        .querySelectorAll(`${this.argumens[0]} .gallery-items`)
        .forEach((items) => {
